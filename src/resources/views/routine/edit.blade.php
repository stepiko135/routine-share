...
<form action="/routine_item/" id="submitItems" class="items" method="POST">
    ...
    <div class="container card">
        <label>時間<input type="time" name="time" value="{{old('time')}}"></label>
        <label>アイテム名<input type="text" name="title" placeholder="(例) コーヒーを飲む" autofocus value="{{old('title')}}"></label>
        <label>説明<textarea name="desc" cols="50" rows="10" placeholder="(例) 濃いめのコーヒーがおすすめです！"
                >{{old('desc')}}</textarea></label>
...
<div class="container card">
    <form action="/routine_item/{{$item->id}}" method="POST">
        ...
        <input type="hidden" name="user_id" value="{{Auth::id()}}">
        <input type="hidden" name="routine_id" value="{{$routine->id}}">
        <label>時間<input type="time" name="time" value="{{$item->time}}"></label>
        <label>アイテム名<input type="text" name="title" placeholder="(例) コーヒーを飲む" autofocus value="{{$item->title}}"></label>
        <label>説明<textarea name="desc" cols="50" rows="10" placeholder="(例) 濃いめのコーヒーがおすすめです！"
                >{{$item->desc}}</textarea></label>
